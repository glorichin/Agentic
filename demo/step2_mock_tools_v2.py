...
# ═══════════════════════════════════════════════════════════
# v2 — PER-ASSET SAFE THRESHOLDS
#
# Each asset type has its own safe-operating limits (different machines,
# different tolerances). These drive sensor_telemetry_api (what counts as a
# breach) AND the anomaly/RUL scoring below, so changing a threshold genuinely
# moves the decision. vibration in g, temperature as % above baseline, motor
# current in amps.
# ═══════════════════════════════════════════════════════════

# temp_c is an ABSOLUTE temperature limit in °C (baseline is ~74°C), so the
# sidebar reading (°C) and the threshold (°C) are the same unit — intuitive.
ASSET_THRESHOLDS = {
    "CNC-Lathe-07":  {"vibration_g": 0.35, "temp_c": 85, "motor_amp": 21},
    "CNC-Lathe-03":  {"vibration_g": 0.40, "temp_c": 86, "motor_amp": 22},
    "Robot-Cell-12": {"vibration_g": 0.50, "temp_c": 88, "motor_amp": 25},
    "Conveyor-B4":   {"vibration_g": 0.55, "temp_c": 90, "motor_amp": 28},
}
DEFAULT_THRESHOLDS = {"vibration_g": 0.45, "temp_c": 87, "motor_amp": 23}


def get_thresholds(asset_id: str) -> dict:
    """Return the safe-operating thresholds for an asset (falls back to a
    sensible default for unknown assets)."""
    return ASSET_THRESHOLDS.get(asset_id, DEFAULT_THRESHOLDS)
...
def sensor_telemetry_api(asset_id: str, window_hours: int = 72) -> str:
    ...
    if asset_id == "CNC-Lathe-07":
        ...
        data = {
            ...
            "vibration_threshold_g":       thr["vibration_g"],
            "vibration_above_threshold":   vib > thr["vibration_g"],
...
def anomaly_detector(asset_id: str, vibration_g: float,
                     ...
                     vib_threshold: float = 0.35,
                     temp_threshold: float = 10,
                     motor_threshold: float = 21) -> str:
    """
    ...
    v2: the three *_threshold args are the asset's safe limits — a signal only
    contributes to the score once it breaches its own threshold. Defaults keep
    v1 behaviour for any caller that doesn't pass them.
    ...
    if vibration_g > vib_threshold:
        contrib = min((vibration_g - vib_threshold) / vib_threshold, 1.0)
        score  += 0.45 * contrib
        contributing.append(
            f"Spindle vibration {vibration_g}g — {round(contrib*100)}% above "
            f"threshold {vib_threshold}g"
        )

    if temp_deviation_pct > temp_threshold:
        contrib = min((temp_deviation_pct - temp_threshold) / 20, 1.0)
        score  += 0.30 * contrib
        contributing.append(
            f"Temperature {round(temp_deviation_pct, 1)}% above baseline "
            f"(limit {temp_threshold}%)"
        )

    if motor_current_amp > motor_threshold:
        contrib = min((motor_current_amp - (motor_threshold - 1)) / 5, 1.0)
        score  += 0.25 * contrib
        contributing.append(
            f"Motor current {motor_current_amp}A — elevated load "
            f"(limit {motor_threshold}A)"
        )
...
def rul_predictor(asset_id: str, vibration_g: float,
                  ...
                  avg_bearing_life_days: int,
                  vib_threshold: float = 0.35) -> str:
    """
    ...
    if vibration_g > vib_threshold:
        ...
        vibration_factor = min(1 + (vibration_g - vib_threshold) / 0.20, 3.0)
...
def root_cause_analyzer(asset_id: str, fault_signals: list,
                        ...
                        primary_signal: str = None) -> str:
    """
    Cross-reference current signals with historical fault patterns
    to surface the most likely root cause and recommended fix.

    v2: the root cause now follows the SIGNAL that triggered the event, so a
    temperature trigger gets a cooling diagnosis and a motor-current trigger
    gets a motor diagnosis — not always "spindle bearing". Falls back to the
    asset's historical pattern when no signal is given.
    ...
    Args:
        ...
        primary_signal:      The signal that triggered the event
                             (spindle_vibration / temperature / motor_current)
    """
    sig_str = str(fault_signals).lower()

    if primary_signal == "temperature" or (
            not primary_signal and "temperature" in sig_str):
        primary     = "Cooling / lubrication fault — spindle overheating"
        secondary   = [
            "Coolant pressure below nominal — possible blocked line or pump wear",
            "Lubrication film breakdown raising friction and temperature",
        ]
        confidence  = 0.79
        fix         = "Inspect coolant system, restore lubrication, check pump seal"
        similar     = 1
    elif primary_signal == "motor_current" or (
            not primary_signal and "motor" in sig_str):
        primary     = "Motor overload / electrical fault — elevated current draw"
        secondary   = [
            "Mechanical binding increasing load on the drive",
            "Possible winding degradation or supply imbalance",
        ]
        confidence  = 0.76
        fix         = "Inspect load path, check alignment/bearings, electrical test"
        similar     = 1
    elif (primary_signal == "spindle_vibration"
          or "bearing" in historical_pattern.lower() or "vibration" in sig_str):
