...
# Disable CrewAI's anonymous usage telemetry BEFORE importing crewai. Otherwise
# it tries to reach telemetry.crewai.com and, if that host is blocked, blocks
# for ~30s with a scary timeout error mid-demo. Must be set before the import.
...
import step2_mock_tools_v2 as mock
...
#
# ── v2 CHANGE ─────────────────────────────────────────────────────
# In v1 the telemetry was hardcoded per asset, so the sidebar "Reading value"
# and "Threshold" had no effect on the agents' decision. In v2 the sidebar
# event flows in: run_pipeline() stores it in _EVENT_OVERRIDE, and _telemetry()
# patches the chosen signal's reading/threshold BEFORE the agents (and every
# analytics tool) see it. Now sliding the value up/down genuinely changes the
# anomaly score → RUL → health → and the final AUTONOMOUS-vs-ESCALATE decision.
# ═══════════════════════════════════════════════════════════════════

# Set by run_pipeline() from the incoming event, then read by _telemetry().
_EVENT_OVERRIDE: dict = {}


def _telemetry(asset_id: str, window_hours: int = 72) -> str:
    """Fetch the asset's telemetry, then overlay the sidebar event so the agents
    reason over the value the USER set (not just the hardcoded mock value).

    Only the selected signal is overridden; the other sensors keep their mock
    readings (multi-sensor reasoning stays intact)."""
    data = json.loads(mock.sensor_telemetry_api(asset_id, window_hours))
    ov = _EVENT_OVERRIDE
    if ov and ov.get("asset_id") == asset_id:
        r = data["readings"]
        ...
        if signal == "spindle_vibration":
            if value is not None:
                r["spindle_vibration_g"] = value
            if thr is not None:
                r["vibration_threshold_g"] = thr
            r["vibration_above_threshold"] = (
                r["spindle_vibration_g"] > r["vibration_threshold_g"])
        elif signal == "temperature":
            if value is not None:
                r["temperature_c"] = value
                base = r.get("baseline_temp_c", 74.0)
                r["temp_deviation_pct"] = round((value - base) / base * 100, 1)
            if thr is not None:
                r["temp_threshold_c"] = thr   # absolute °C limit
        elif signal == "motor_current":
            if value is not None:
                r["motor_current_amp"] = value
            if thr is not None:
                r["motor_threshold_amp"] = thr
        data["readings"] = r
    return json.dumps(data, indent=2)


def _anomaly(asset_id: str, r: dict) -> str:
    """Run the anomaly model with the asset's (possibly overridden) thresholds,
    so a signal only counts once it breaches its own limit. The temperature
    limit is stored in °C but the model scores % deviation, so convert here."""
    base = r.get("baseline_temp_c", 74.0)
    temp_thr_c = r.get("temp_threshold_c")
    if temp_thr_c is not None and base:
        temp_thr_pct = (float(temp_thr_c) - base) / base * 100
    else:
        temp_thr_pct = 10
    return mock.anomaly_detector(
        asset_id,
        r["spindle_vibration_g"], r["temp_deviation_pct"], r["motor_current_amp"],
        r.get("vibration_threshold_g", 0.35),
        temp_thr_pct,
        r.get("motor_threshold_amp", 21),
    )
...
def sensor_telemetry_api(asset_id: str, window_hours: int = 72) -> str:
    ...
    return _telemetry(asset_id, window_hours)
...
def anomaly_detector(asset_id: str) -> str:
    ...
    r = json.loads(_telemetry(asset_id))["readings"]
    return _anomaly(asset_id, r)
...
def asset_health_scorer(asset_id: str) -> str:
    ...
    r   = json.loads(_telemetry(asset_id))["readings"]
    s   = json.loads(mock.maintenance_history_api(asset_id))["summary"]
    an  = json.loads(_anomaly(asset_id, r))
...
def root_cause_analyzer(asset_id: str) -> str:
    ...
    s  = json.loads(mock.maintenance_history_api(asset_id))["summary"]
    an = json.loads(_anomaly(asset_id, r))
    signal = _EVENT_OVERRIDE.get("signal") if _EVENT_OVERRIDE else None
    return mock.root_cause_analyzer(
        asset_id, an["contributing_signals"], s["pattern_flag"], signal)
...
def build_agents(llm: LLM) -> dict:
    ...
    cost = Agent(
        ...
        backstory=(
            ...
            "cost_of_failure_now = the cost of an UNPLANNED failure = expected "
            "emergency downtime (assume ~14 hours for a CNC machine) x the hourly "
            "downtime cost (daily_downtime_cost / 24). Do NOT multiply by the "
            "remaining life — that overstates it. cost_of_planned_maintenance = "
            "parts_cost + labour; roi = cost_of_failure_now - "
            "cost_of_planned_maintenance. You assign a "
...
def _is_severe(event: dict) -> bool:
    """Decide whether this event warrants an EMERGENCY shutdown (which forces a
    human escalation) versus a mild, early-warning issue that can be handled by
    planned maintenance (the autonomous path).

    Severe = the reading is more than 20% over its limit. All signals now use
    the same units as their threshold (vibration g, temperature °C, current A),
    so it's one uniform comparison. This is what lets the demo branch between
    ESCALATE and AUTONOMOUS."""
    ...
    if threshold <= 0:
        return value > 0
    return (value - threshold) / threshold >= 0.20
...
def build_crew(event: dict, agents: dict, task_callback=None) -> Crew:
    ...
    aid = event["asset_id"]
    severe = _is_severe(event)   # drives shutdown proposal → ESCALATE vs AUTONOMOUS
    ...
    if severe:
        gov_actions = (
            ...
            f"actions proposed for asset {aid}: (1) draft a work order, "
            "(2) notify the maintenance lead, (3) update the dashboard, and "
            "(4) a recommended EMERGENCY machine shutdown window for the urgent "
            "repair. Call policy_validator for each and return the decision per "
            "action."
        )
    else:
        gov_actions = (
            f"This is a MILD, early-warning issue for asset {aid} — the reading "
            "is only slightly over its limit, so the repair can wait for the "
            "next PLANNED maintenance window and NO emergency shutdown is "
            "proposed. Using the Reliability, Process and Cost assessments, "
            "review the proposed actions: (1) draft a PLANNED work order, "
            "(2) notify the maintenance lead, (3) update the dashboard. Call "
            "policy_validator for each and return the decision per action."
        )

    governance_task = Task(
        description=gov_actions,
        expected_output=(
    ...
    if severe:
        shutdown_note = (
            "This event is SEVERE and an emergency machine shutdown is being "
            "proposed — that requires a human, so you must ESCALATE."
        )
    else:
        shutdown_note = (
            "This is a MILD event handled by PLANNED maintenance — NO machine "
            "shutdown is proposed. So if Priority < 0.7 and no action needs "
            ">$10,000 approval and Governance did not REJECT, you may decide "
            "AUTONOMOUS and execute the approved actions."
        )
    ...
    orchestrator_task = Task(
        description=(
            ...
            "anything the formula misses. " + shutdown_note + " Decide AUTONOMOUS "
            "vs ESCALATE per the hard constraints (a CNC machine is never taken "
            "offline autonomously; repairs > $10,000 always escalate; a Governance "
            "REJECTED stops everything). Then EXECUTE the autonomous-approved "
            "actions by calling work_order_generator, notification_service and "
            "dashboard_updater."
        ),
        expected_output=(
...
def run_pipeline(event: dict = None, on_task_complete=None) -> dict:
    ...
    # v2: publish the sidebar event so _telemetry() can overlay the user's
    # reading value / threshold onto the telemetry the agents reason over.
    global _EVENT_OVERRIDE
    _EVENT_OVERRIDE = {
